...
static int	check_i_opt(t_lex *lex, char **arg)
{
	int i;
...
static int	check_u_opt(t_lex *lex, char **arg, char **save)
{
	int i;
	...
	if (arg[i][0] == '-' && arg[i][1] == 'u' && !arg[i][2] && arg[i + 1])
	{
		...
		arg++;
		save = check_arg(lex->info->lst, *arg, save);
		if (!save || !(*save))
			print_error_opt(*arg);
		arg++;
		if (*arg)
...
char		**deal_with_opt(t_lex *lex, char **arg)
{
	char	**save;
	...
	if (!save)
		return (NULL);
	if (check_u_opt(lex->info, arg, save) == 1)
	{
		if (check_i_opt(lex->info, arg) == 1)
		{
